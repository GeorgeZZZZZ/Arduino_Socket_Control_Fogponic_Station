...
enum commandHeaderType
{
  noneHeader
  , FogGenerator_On_TimeChange
  , FogGenerator_Off_TimeChange
};
...
// if not using any net work function then set this value to true
bool NotUsingAnyNetWorkFunction = true;
...
// value using in state machine
int nextOnTime = 0;
int nextOffTime = 0;
commandHeaderType currentHeader;
...
bool FogGeneratorOnRollover = false;
unsigned int FogGeneratorDefultOnTime = 60000;  // 1000 for 1000ms
unsigned int FogGeneratorOnTime = 0;
unsigned long FogGeneratorOnTimerCache = 0;

bool FogGeneratorOffRollover = false;
unsigned int FogGeneratorDefultOffTime = 30000;
unsigned int FogGeneratorOffTime = 0;
unsigned long FogGeneratorOffTimerCache = 0;
...
// debug mode define
bool tcpMessageDebug = false;

//-----------------

void setup() {
  // initiallize values
  FogGeneratorOnTime = FogGeneratorDefultOnTime;
  FogGeneratorOffTime = FogGeneratorDefultOffTime;
  valueInDefultFlag = 1;
...
{
  nextOnTime = nextOffTime = 0;  // clean temp stroge valuable
  currentHeader = noneHeader;
}
...
uint8_t HeaderAnalysis (String _str)
{
  if (!tcpMessageDebug)
  {
    if (_str == "TOf")
    {
      currentHeader = FogGenerator_Off_TimeChange;
    }
    else if (_str == "TOn")
    {
      currentHeader = FogGenerator_On_TimeChange;
    }
    else
    {
    ...
    return sm_pi_workingFine; // every thing is fine
  }
  else
  {
...
uint8_t ContentAnalysis (String _str)
{
  if (!tcpMessageDebug)
  {
    ...
    {
      case FogGenerator_On_TimeChange:
        nextOnTime = Convert_String_to_Int(_str);
      break;
    
      case FogGenerator_Off_TimeChange:
        nextOffTime = Convert_String_to_Int(_str);
      break;
...
void CommandApplyAtEnd ()
{
  if (!tcpMessageDebug)
  {
    if (nextOnTime != 0) FogGeneratorOnTime = abs(nextOnTime) * 60000;  // 60000 for 1mins
    if (nextOffTime != 0) FogGeneratorOffTime = abs(nextOffTime) * 60000;
  }
  else
  {
...
void MessageToBePrintToSerialConstantly()
{
  ...
  if (TimeCounter (SerialMessagePrintTimerCache, 5000, serialMessageTimerRollover))  // sent message every 3s
  {
...
bool fogGeneratorNextState = false;
bool foggeneratorCurrentState = false;
void FogGeneratorManagement ()
{
  fogGeneratorNextState = false;
  digitalWrite(FogGeneratorPin, LOW); // turn off fog generator // in ESP13, low meaning low for pin
  if (FogGeneratorOffTimerCache == 0) FogGeneratorOffTimerCache = millis(); // start countting turnning off time
  if (millis() - FogGeneratorOffTimerCache < 0)  // rollover preventive, if rollover
  { 
    FogGeneratorOffTimerCache = 4294967295 - FogGeneratorOffTimerCache; // calculate already passed time
    FogGeneratorOffRollover = true;
  }
  if (TimeCounter (FogGeneratorOffTimerCache, FogGeneratorOffTime, FogGeneratorOffRollover))  // off time reached
  {
    fogGeneratorNextState = true;
    digitalWrite(FogGeneratorPin, HIGH); // turn on fog generator
    if (FogGeneratorOnTimerCache == 0)  FogGeneratorOnTimerCache = millis();  // start countting led turnning on time
    if (millis() - FogGeneratorOnTimerCache < 0)
    { 
      FogGeneratorOnTimerCache = 4294967295 - FogGeneratorOnTimerCache;
      FogGeneratorOnRollover = true;
    }
    if (TimeCounter (FogGeneratorOnTimerCache, FogGeneratorOnTime, FogGeneratorOnRollover))  // on time reached
    {
      FogGeneratorOffTimerCache = FogGeneratorOnTimerCache = 0; // reset timer
      FogGeneratorOffRollover = FogGeneratorOnRollover = false;
    }
  }

  // serial message for debug
  if (foggeneratorCurrentState != fogGeneratorNextState)
  {
    foggeneratorCurrentState = fogGeneratorNextState;
    if(fogGeneratorNextState) Serial.println("fogGeneratorState is on.");
    else  Serial.println("fogGeneratorState is off.");
  }
}
